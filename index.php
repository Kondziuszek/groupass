<?php
    ...
    if (($_SESSION['do'] == 0 and !empty($_POST['uid'])) or ($_SESSION['do'] == 1 and !empty($_SESSION['uid']))) {
        if ((!empty($_SESSION['uid']) and strlen($_SESSION['uid']) == 28 and $_SESSION['uid'][27] == "=") or (strlen($_POST['uid']) == 28 and $_POST['uid'][27] == "=")) {
            ...
            try {
                TeamSpeak3::init();
                $ts3 = TeamSpeak3::factory("serverquery://". $config['teamspeak']['loginname'] .":". $config['teamspeak']['loginpass'] ."@". $config['teamspeak']['ip'] .":". $config['teamspeak']['queryport'] ."/?server_port=". $config['teamspeak']['serverport'] ."&nickname=". urlencode('(RANGA) '.$config['teamspeak']['displayname']) ."");
